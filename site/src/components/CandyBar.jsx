...
import { isImagePreloaded, preloadImages } from '../services/prefetch';
import './CandyBar.css';
import './Skeleton.css';
...
export default function CandyBar() {
  ...
  const [brochureReady, setBrochureReady] = useState(() => isImagePreloaded(brochureSlides[0]));
  const [loadedSlides, setLoadedSlides] = useState(
    () => new Set(brochureSlides.map((src, index) => (isImagePreloaded(src) ? index : null)).filter((value) => value !== null))
  );
  const [loadedCatalogImages, setLoadedCatalogImages] = useState(
    () => new Set(menuItems.map((item) => item.image).filter((src) => isImagePreloaded(src)))
  );
  ...
  useEffect(() => {
    ...
    brochureSlides.forEach((src, index) => {
      const img = new Image();
      img.decoding = 'async';
    ...
    preloadImages([brochureSlides[0], brochureSlides[1], brochureSlides[brochureSlides.length - 1]], 5000)
      .then(() => {
        if (isMounted) setBrochureReady(true);
      });
    ...
    menuItems.forEach((item) => {
      ...
      img.onload = () => {
        if (!isMounted) return;
        setLoadedCatalogImages((prev) => {
          if (prev.has(item.image)) return prev;
          const next = new Set(prev);
          next.add(item.image);
          return next;
        });
    ...
    preloadImages(menuItems.map((item) => item.image), 7000).then(() => {
      if (!isMounted) return;
      setLoadedCatalogImages(new Set(menuItems.map((item) => item.image)));
    });

    return () => {
      isMounted = false;
    };
  }, [brochureSlides, menuItems]);
  ...
  const currentSlideLoaded = loadedSlides.has(currentSlide);
        ...
        <div className="brochure-carousel-wrapper">
          ...
          <div className="brochure-carousel">
            ...
            <div className="carousel-slide-container">
              {(!brochureReady || !currentSlideLoaded) && (
                <div className="brochure-slide-skeleton skeleton" aria-hidden="true" />
              )}
              <img 
                src={brochureSlides[currentSlide]} 
                alt={`Brochure Page ${currentSlide + 1}`} 
                className={`brochure-slide-img ${brochureReady && currentSlideLoaded ? 'loaded' : 'loading'}`}
                loading="eager"
            ...
            {menuItems.map((item, index) => (
              <div key={index} className="candy-card">
                <div className="candy-card-img-wrapper">
                  {!loadedCatalogImages.has(item.image) && (
                    <div className="candy-card-img-skeleton skeleton" aria-hidden="true" />
                  )}
                  <img
                    src={item.image}
                    alt={item.name}
                    className={`candy-card-img ${loadedCatalogImages.has(item.image) ? 'loaded' : 'loading'}`}
                    loading="lazy"
                    decoding="async"
                  />
                </div>
